...
my ($unid,$avpid,$cmdperm,$cmddeny,$argperm,$argdeny) = ('','','','','','');
...
$cmdperm = $query->param('cmdperm');
$cmddeny = $query->param('cmddeny');
$argperm = $query->param('argperm');
$argdeny = $query->param('argdeny');

#--- remove whitespace at the end and begin of the line, replace whitespaces by one whitespace  ---
$unid = join(" ", split " ", $unid);
$avpid = join(" ", split " ", $avpid);
$cmdperm = join(" ", split " ", $cmdperm);
$cmddeny = join(" ", split " ", $cmddeny);
$argperm = join(" ", split " ", $argperm);
$argdeny = join(" ", split " ", $argdeny);

#--- Null row ------
if ($avpid eq ''){
    print $query->redirect("./cmd.pl?p=$pagenumber&w=$switch") ; 
    exit;
}
...
if (PGRES_CONNECTION_OK == $conn->status) {
    ...
    if (PGRES_CONNECTION_OK == $sec_conn->status) {
        ...
        if ($unid eq ''){
            ...
            $result = $conn->exec("SELECT MAX(unid) FROM tacppd_v1_cmd;");
            ...
            ######>>> INSERT NEW USER ##############
            $cmd = "INSERT INTO tacppd_v1_cmd(avpid,cmdperm,cmddeny,argperm,argdeny,unid) 
                    VALUES('$avpid','$cmdperm','$cmddeny','$argperm','$argdeny',$unid);";
        ...
        #--- edit record -
            ...
            if($delflag == 1){
                ######>>> DELETE USER ##############
                $cmd = "DELETE FROM tacppd_v1_cmd 
                          WHERE unid = $unid;";
         
            }else{
         
                ######>>> UPDATE USER ##############
                $cmd = "UPDATE tacppd_v1_cmd 
                         SET avpid='$avpid', cmdperm='$cmdperm', cmddeny='$cmddeny', argperm='$argperm', argdeny='$argdeny'
                          WHERE unid = $unid;";
            }

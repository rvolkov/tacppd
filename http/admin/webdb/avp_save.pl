...
my ($unid,$avpid,$service,$delflag,$protocol,$avp) = ('','','','','','');
...
$service = join(" ", split " ", $service);
$protocol = join(" ", split " ", $protocol);
$avp = join(" ", split " ", $avp);


#--- Null row ------
if ($avpid eq ''){
    print $query->redirect("./avp.pl?p=$pagenumber&w=$switch") ; 
    exit;
}
...
if (PGRES_CONNECTION_OK == $conn->status) {
    ...
    if (PGRES_CONNECTION_OK == $sec_conn->status) {
        ...
        if ($unid eq ''){
            ...
            $result = $conn->exec("SELECT MAX(unid) FROM tacppd_v1_avp;");
            ...
            ######>>> INSERT NEW USER ##############
            $cmd = "INSERT INTO tacppd_v1_avp(avpid,service,protocol,avp,unid) 
                    VALUES('$avpid','$service','$protocol','$avp',$unid);";
        ...
        #--- edit record -
            ...
            if($delflag == 1){
                ######>>> DELETE USER ##############
                $cmd = "DELETE FROM tacppd_v1_avp 
                          WHERE unid = $unid;";

            }else{

                ######>>> UPDATE USER ##############
                $cmd = "UPDATE tacppd_v1_avp 
                         SET avpid='$avpid', service='$service', protocol='$protocol', avp='$avp' 
                          WHERE unid = $unid;";
            }

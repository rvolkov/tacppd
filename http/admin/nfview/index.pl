...
print"<TD VALIGN=TOP class=SiteTop1>";
         ...
         if($flag){
             print"<SPAN class=SelectLang>$dict{lblSwitchLang}</SPAN><br>$selectlang<br><br>";
         }else{
             #--- Select language ---
...
print"<TD VALIGN=TOP class=SiteTop2>
         ...
         if($flag){
             ...
             print"<SPAN class=SelectLang>$dict{LIST_BIG}</SPAN>";
         }
      print"</TD>";
print"<TD VALIGN=TOP class=SiteTop3>
         <TABLE><TR>
          ...
          if($flag){
              print"<TD WIDTH=140 colspan=2 class=inner><A HREF=./index.pl class=Link $dict{LIST_BIG}\">$dict{LIST}</A></TD>";
              print"<TD WIDTH=140 colspan=2 class=inner><SPAN class=Link>$dict{NEW}</SPAN></TD>";
          }else{
              print"<TD WIDTH=140 colspan=2 class=inner><SPAN class=Link>$dict{LIST}</SPAN></TD>";
              print"<TD WIDTH=140 colspan=2 class=inner><A HREF=./index.pl?f=1 class=Link TITLE=\"$dict{NEW_BIG}\">$dict{NEW}</A></TD>";
          }
...
if (PGRES_CONNECTION_OK == $conn->status) {
    ...
    if($flag){
        ...
        if($usernet && $name){
            ...
            $result = $conn->exec("SELECT * FROM tacppd_v1_nfuser WHERE usernet='$usernet'");
            $row = $result->ntuples; 

            #-- Update ---
            if($row > 0){
                $result = $conn->exec("UPDATE tacppd_v1_nfuser SET name='$name', comment='$memo' WHERE usernet='$usernet'");
                $error = $conn->errorMessage unless PGRES_TUPLES_OK eq $result->resultStatus;
                print_error_and_exit($dict{ERROR_5},$conn,$error,$dict{ERROR},$dict{CAUSE}) if $error ne '';

            #-- Insert ---
            }else{
                $result = $conn->exec("INSERT INTO tacppd_v1_nfuser (usernet,name,comment) VALUES ('$usernet','$name','$memo')");
                $error = $conn->errorMessage unless PGRES_TUPLES_OK eq $result->resultStatus;
                print_error_and_exit($dict{ERROR_5},$conn,$error,$dict{ERROR},$dict{CAUSE}) if $error ne '';
        ...
        #-- *** delete row *** ---
        my $dflag = $query->param("d");
        if($usernet && $dflag == 1){
            $result = $conn->exec("DELETE FROM tacppd_v1_nfuser WHERE usernet='$usernet'");
            $error = $conn->errorMessage unless PGRES_TUPLES_OK eq $result->resultStatus;
            print_error_and_exit($dict{ERROR_4},$conn,$error,$dict{ERROR},$dict{CAUSE}) if $error ne '';
        }
        ...
        print"<TD VALIGN=TOP>";
          print"$dict{USERNAME}: ";
          print"<INPUT TYPE=text AUTOCOMPLETE=\"off\" SIZE=20 MAXLENGTH=60 NAME=name CLASS=PlaneText VALUE=\"$name\">";
          print" &nbsp; ";
          print"IP: ";
          print"<INPUT TYPE=text AUTOCOMPLETE=\"off\" SIZE=20 MAXLENGTH=60 NAME=ip CLASS=PlaneText  VALUE=\"$usernet\">";
          print" &nbsp; ";
          print"$dict{COMMENT}: ";
          print"<INPUT TYPE=text AUTOCOMPLETE=\"off\" SIZE=40 NAME=memo CLASS=PlaneText>";
          ...
          print"<INPUT TYPE=hidden NAME=f VALUE=$flag>";
          print"&nbsp; <INPUT TYPE='submit' VALUE=\"$dict{cmdSave}\" class='MiniCmd'>";
          print" &nbsp; <INPUT TYPE='button' VALUE=\"$dict{cmdClear}\" class='MiniCmdGr' onClick=\"document.ViewForm.name.value=''; document.ViewForm.ip.value=''; document.ViewForm.memo.value='';\">";
        print"</TD>";
        print"</FORM>";
        print"</TR></TABLE>";
        ##################################################################
    ...
    print"<SPAN class=SelectLang>$dict{INDEX_SEARCH}: <b>$search</b></SPAN>" if $search ne '';
      ...
      #--- page line ------
      &print_page_line('index.pl',1,$pagenumber,$cicle,$selectpage,$switch,$nextpage,$dict{PAGE},$dict{NEXT_PAGE},$dict{USERNAME},$dict{cmdSearch},$flag,$search);
    ...
    print"<TR>";
     ...
     if($search ne ''){
         ...
         print"<SPAN class=Link>$dict{USERNAME}</SPAN>&nbsp; 
             <A HREF=./index.pl?w=2&f=$flag class=Link TITLE=\"$dict{SORT_DESC}\">&#150;</A> 
             <A HREF=./index.pl?w=1&f=$flag class=Link TITLE=\"$dict{SORT_ASC}\">+</A>";
     }else{
         $cmd = print_sort_table_header($switch,$cmd,1,2,'index.pl',$dict{USERNAME},$dict{SORT_DESC},$dict{SORT_ASC},'name','','',$flag);
     }
     print"</TD>";
     print"<TD class=TitleTable0></TD>";
     ...
     #--- ip (+) ---
     print"<TD class=TitleTable1 VALIGN=TOP>";
     $cmd = print_sort_table_header($switch,$cmd,3,4,'index.pl',$dict{IP},$dict{SORT_DESC},$dict{SORT_ASC},'usernet','','',$flag);
     print"</TD>";
     print"<TD class=TitleTable0></TD>";

     #--- comment (+) ---
     print"<TD class=TitleTable1 VALIGN=TOP>";
     $cmd = print_sort_table_header($switch,$cmd,5,6,'index.pl',$dict{COMMENT},$dict{SORT_DESC},$dict{SORT_ASC},'comment','','',$flag);
     print"</TD>";
     print"<TD class=TitleTable0></TD>";
    ...
    if($row > 0){
        ...
        for(my $i=0; $i<$row; $i++){
            ...
            $print_usernet = sprintf("%-20s",$print_usernet);

            if($flag){
                print"   <A href=\"javascript:paste('$usernet','$name','$pagenumber','$search','$switch',window.document.ViewForm)\" class=Link>$print_name $print_usernet</A> <A HREF=./index.pl?ip=$usernet&f=$flag&d=1&p=$pagenumber&w=$switch&s=$search class=Link><IMG SRC=./images/del.gif BORDER=0 TITLE='$dict{cmdDel} $usernet'> </A>$comment<br>";
            }else{
    ...
    #--- page line ------
    print"<p>";
    &print_page_line('index.pl',0,$pagenumber,$cicle,$selectpage,$switch,$nextpage,$dict{PAGE},$dict{NEXT_PAGE},$dict{USER},$dict{cmdSearch},$flag,$search);
...
sub print_table_with_traffic{
    ...
    my ($spider,$sp_yyyy,$sp_mm,$sp_dd,$sp_hh,$dd,$flag,$formprint);
    ...
    for(;;){
        ...
        print"<TR class=listtable>";
        print"<TD class=nocolor><A HREF='javascript:open_child(\"./moredetail.pl?ip=$ip&d=$start_date\",650,600);void(0);' class=Link>$sp_yyyy-$sp_mm-$sp_dd</A></TD>";

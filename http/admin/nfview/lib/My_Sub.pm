...
require Exporter; 
@ISA  =  qw(Exporter); 
@EXPORT  =  qw(get_cur_datetime get_num_date print_page_line print_error_and_exit 
               print_sort_table_header get_year_month_etc_from_date);
use Time::Local;
use Time::localtime;
use locale;


sub get_year_month_etc_from_date{
   my ($time) = @_;
   my($obj_time) = localtime($time);
   my $mm = sprintf("%02d", $obj_time->mon() + 1 );
   my $yyyy = $obj_time->year() + 1900;
   my $dd = sprintf("%02d", $obj_time->mday() );
   my $hh = sprintf("%02d", $obj_time->hour() );
   return($yyyy,$mm,$dd,$hh); 
}


sub print_sort_table_header{
    my($switch,$cmd,$f1,$f2,$script,$title,$sort_desc,$sort_asc,$field,$field2,$reverse,$flag) = @_;
    ...
    if($switch == $f1){
        ...
        print"<SPAN class=Link>$title&nbsp;</SPAN>
              <A HREF=./$script?w=$f2&f=$flag class=Link TITLE=\"$sort_desc\">&#150;</A> 
    ...
    #-- [-] --
        ...
        print"<SPAN class=Link>$title&nbsp; &#150;</SPAN>
              <A HREF=./$script?w=$f1&f=$flag class=Link TITLE=\"$sort_asc\">+</A>";

    #-- [+-] --
    }else{
        #-- (+) --
        if($reverse eq ''){
            print"<A HREF=./$script?w=$f1&f=$flag class=Link TITLE=\"$sort_asc\">$title</A>&nbsp; 
                  <A HREF=./$script?w=$f2&f=$flag class=Link TITLE=\"$sort_desc\">&#150;</A> 
                  <A HREF=./$script?w=$f1&f=$flag class=Link TITLE=\"$sort_asc\">+</A>";
        #-- (-) --
        }else{
            print"<A HREF=./$script?w=$f2&f=$flag class=Link TITLE=\"$sort_desc\">$title</A>&nbsp; 
                  <A HREF=./$script?w=$f2&f=$flag class=Link TITLE=\"$sort_desc\">&#150;</A> 
                  <A HREF=./$script?w=$f1&f=$flag class=Link TITLE=\"$sort_asc\">+</A>";
        }
    }
...
sub print_page_line{
    my($scriptname,$flag,$pagenumber,$cicle,$selectpage,$switch,
       $nextpage,$dict_page,$dict_nextpage,$dict_user,$dict_search,$eflag,$search) = @_;
    my $viewpage;
    ...
    print"<TR><TD VALIGN=TOP class=SiteTop3>";
     ...
     for ($pagenumber = 1; $pagenumber < $cicle; $pagenumber++){
         if($selectpage eq $pagenumber){
             ...
             $viewpage = $pagenumber;
             print" <A HREF=./$scriptname?p=$pagenumber&w=$switch&f=$eflag&s=$search class=Link> $viewpage </A> ";
         }
     } #for
     if($selectpage < $nextpage ){
         print" <A HREF=./$scriptname?p=$nextpage&w=$switch&f=$eflag&s=$search class=Link title=\"$dict_nextpage\"> &nbsp; &nbsp; &gt; &nbsp; &nbsp; </A> ";
     }
    print"</TD>";

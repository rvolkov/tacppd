...
db_destroy() {
  ...
  for(list<connection_struct*>::iterator i=con.begin();i!=con.end() && !con.empty();) {
    tmp = *i;
    i = con.erase(i);
    if(tmp->SQL_res) PQclear(tmp->SQL_res);
    if(tmp->SQL_conn) PQfinish(tmp->SQL_conn);
    delete tmp;
  }
  unlock();
...
//cerr<<"postgresql driver connection num = "<<con.size()<<endl;

  char p[20];
  snprintf(p,sizeof(p),"%d",port);
  tmp->SQL_conn = PQsetdbLogin(host, p, NULL, NULL, dbase, login, pwd);
  if(PQstatus(tmp->SQL_conn) == CONNECTION_BAD) {
    snprintf(derr,DB_MODULE_ERRLEN,"Connection to database %s failed: %s", dbase,
      PQerrorMessage(tmp->SQL_conn));
    PQfinish(tmp->SQL_conn); // finish connection
    delete tmp;
    unlock();
    return 0;
  }
  ...
  // set datetme style
  if(!sdb_query(tmp->SQL_conn,"SET DateStyle TO 'ISO'",rerr)) {
...
db_close(int num) {
  ...
  for(list<connection_struct*>::iterator i=con.begin();i!=con.end();i++) {
    ...
    if(ctmp->num == num) {
      res = true;
      if(ctmp->SQL_res) PQclear(ctmp->SQL_res);
      if(ctmp->SQL_conn) PQfinish(ctmp->SQL_conn);
      i = con.erase(i);
      delete ctmp;
      break;
    }
